...
pub async fn in_check(
    headers: HeaderMap
) -> Result<Option<AuUser>, Option<String>> {
    match headers.get("Cookie") {
        None => return Ok(None),
        Some(expr) => expr,
    };
    ...
    let mut user = AuUser::default();
    let mut claims: JWTClaims<AuUser>;
    ...
    for i in &a {
        if i.split("=").next() == Some("dialogue") {
            ...
            pat = "./static/public_key/user/".to_string() + &dialogue + ".bin";
        }
    }
    for i in a {
        if i.split("=").next() == Some("visit") {
            visit.push_str(i.split("=").last().unwrap());
            let key: &[u8] = &fs::read(&pat).unwrap();
            let k = ES256PublicKey::from_bytes(key);
            claims = k.unwrap().verify_token::<AuUser>(&visit, None).unwrap();
            user = claims.custom;
        }
    }
    Ok(Some(user))
